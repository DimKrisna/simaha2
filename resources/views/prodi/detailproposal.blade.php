    ...
    <div class="container">
        ...
        <div class="form-container mt-4">
            ...
            <div class="d-flex align-items-center mb-2">
                <div class="w-20 fw-bold">Status Pengajuan Tingkat Prodi</div>
                <div class="me-2">:</div>
            </div>
<form action="{{ route('update-proposal', ['id' => $proposal->id_proposal]) }}" method="POST">
    ...
    <div class="form-group mb-4 p-3 border rounded shadow-sm bg-light">
        <div class="row align-items-center mb-3">
            ...
            <div class="col-md-9">
                <select class="form-select w-50" id="status_kaprodi" name="status_kaprodi" required>
...
<script>
    document.getElementById('status_kaprodi').addEventListener('change', function() {
        var catatanProdi = document.getElementById('catatan_prodi');
        if (this.value === 'Revisi' || this.value === 'Tolak') {
            catatanProdi.disabled = false;  // Enable textarea
        } else {
            catatanProdi.disabled = true;   // Disable textarea
            catatanProdi.value = '';        // Clear textarea if disabled
        }
    });
</script>
@endsection
