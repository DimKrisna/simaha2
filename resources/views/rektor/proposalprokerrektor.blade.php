...
@section('content')
    <div class="container">
        ...
        <form method="GET" action="{{ route('proposalkegiatanrektor') }}" class="d-flex justify-content-left mb-3">
            <div class="input-group" style="max-width: 300px; gap: 8px;">
                <input type="text" name="search" class="form-control" placeholder="Cari..." value="{{ request('search') }}">
                <button type="submit" class="btn btn-primary">Cari</button>
                <a href="{{ route('proposalkegiatanrektor') }}" class="btn btn-secondary">Reset</a>
            </div>
        </form>
