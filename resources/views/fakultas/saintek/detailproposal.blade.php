...
@section('content')
    <div class="container">
        ...
        <div class="form-container mt-4">
            ...
            <form action="{{ route('update.proposalFakultas', ['id' => $proposal->id_proposal]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <textarea id="latar_belakang" name="latar_belakang" rows="4">{{ $proposal->latar_belakang }} (Tuliskan revisi jika ada, jika tidak silahkan hapus bagian ini) </textarea>

                <label for="deskripsi_kegiatan">Deskripsi Kegiatan:</label>
                <textarea id="deskripsi_kegiatan" name="deskripsi_kegiatan" rows="4">{{ $proposal->deskripsi_kegiatan }} (Tuliskan revisi jika ada, jika tidak silahkan hapus bagian ini)</textarea>

                <label for="tujuan_kegiatan">Tujuan Kegiatan:</label>
                <textarea id="tujuan_kegiatan" name="tujuan_kegiatan" rows="4">{{ $proposal->tujuan_kegiatan }} (Tuliskan revisi jika ada, jika tidak silahkan hapus bagian ini)</textarea>

                <label for="manfaat_kegiatan">Manfaat Kegiatan:</label>
                <textarea id="manfaat_kegiatan" name="manfaat_kegiatan" rows="4">{{ $proposal->manfaat_kegiatan }} (Tuliskan revisi jika ada, jika tidak silahkan hapus bagian ini)</textarea>
                ...
                <div class="button-container">
                    <!-- Tombol Revisi -->
                    <form action="{{ route('update.proposalFakultas', ['id' => $proposal->id_proposal]) }}" method="POST" style="display: inline;">
                        @csrf
                        @method('PUT')
                        <button type="submit" class="btn btn-secondary" name="revisi" value="1">
                            <i class="lni lni-spinner-arrow"></i> Revisi
                        </button>
                    </form>
              ...
              <!-- Tombol ACC -->
              <form action="{{ route('acc.proposal.fakultas', $proposal->id_proposal) }}" method="POST" style="display: inline;">
                @csrf
                @method('PUT')
